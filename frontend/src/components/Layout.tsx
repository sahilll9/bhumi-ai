...
export default function Layout({ children }: LayoutProps) {
  ...
  return (
    <div className="min-h-screen">
      ...
      <nav className="fixed top-6 left-1/2 -translate-x-1/2 z-50 w-[98%] max-w-7xl">
        <div className="glass-card !bg-white/40 backdrop-blur-xl px-6 py-3 rounded-2xl flex justify-between items-center border border-white/40 shadow-2xl">
          <Link to="/" className="flex items-center gap-2">
            <div className="w-10 h-10 bg-bhumi-dark rounded-xl flex items-center justify-center">
              <span className="text-white font-black text-xl">B</span>
            </div>
            <span className="text-xl font-bold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-bhumi-dark to-bhumi-deep hidden sm:block">
              Bhumi AI
            </span>
          </Link>
          ...
          <div className="flex items-center gap-1">
            <div className="hidden lg:flex items-center gap-0.5 mr-2">
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    key={item.path}
                    to={item.path}
                    className={`nav-link px-2.5 xl:px-3 py-2 rounded-xl text-[13px] font-bold transition-all flex items-center gap-1.5 ${isActive
                      ? 'bg-bhumi-dark text-white'
                      : 'text-earth-900/70 hover:text-earth-900 hover:bg-white/50'
                      }`}
                  >
              ...
              {user && (
                ...
                >
                  <button
                    className={`nav-link px-2.5 xl:px-3 py-2 rounded-xl text-[13px] font-bold transition-all flex items-center gap-1.5 ${location.pathname.startsWith('/marketplace')
                      ? 'bg-bhumi-dark text-white'
                      : 'text-earth-900/70 hover:text-earth-900 hover:bg-white/50'
                      }`}
                  >
                    ...
                    {t('Marketplace')}
                    <ChevronDown className={`w-3 h-3 transition-transform ${isMarketplaceOpen ? 'rotate-180' : ''}`} />
                  </button>
              ...
              <div className="flex items-center ml-2 border-l border-earth-900/10 pl-2">
                <select
                  ...
                  className="bg-transparent text-[11px] font-black outline-none cursor-pointer text-earth-900 uppercase tracking-wider"
                >
            ...
            {user ? (
              <div className="flex items-center gap-2 pl-2 lg:pl-4 border-l border-earth-900/10">
                <div className="hidden sm:block text-right">
                  <p className="text-xs font-bold text-earth-900 mb-0">{user.full_name}</p>
                  <p className="text-[10px] text-earth-800/50 uppercase tracking-widest">{user.role}</p>
                </div>
      ...
      {/* Minimalist Footer */}
      <footer className="mt-auto py-12 text-center relative z-10 overflow-hidden">
        <div className="max-w-2xl mx-auto px-4">
          <div className="w-12 h-1 bg-bhumi-primary/20 mx-auto mb-8 rounded-full" />
          <p className="text-earth-800/40 text-sm font-medium tracking-wide">
            POWERED BY ADVANCED RURAL INTELLIGENCE
          </p>
          <div className="mt-4 flex justify-center gap-8 text-xs font-bold text-earth-800/20 uppercase tracking-widest">
            <span>© 2024 BHUMI AI</span>
            <span className="hover:text-bhumi-primary transition-colors cursor-pointer">Privacy</span>
            <span className="hover:text-bhumi-primary transition-colors cursor-pointer">Support</span>
          </div>
        </div>
      </footer>
    </div>
